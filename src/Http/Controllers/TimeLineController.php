...
class TimeLineController extends Controller
{
    ...
    public static function showData(Request $request)
    {
        ...
        if ($request->session()->get('searchKeys') == null && $request['field0'] == null)
            return Redirect::to('/' . strtolower(config('yarm.sys_name')) . '/visualisations/searchForm?form=TimeLine')->with('alert-danger', trans('ERROR: No search result.'));
        ...
        try {
            ...
            return Redirect::to('/' . strtolower(config('yarm.sys_name')) . '/visualisations/searchForm?form=TimeLine')->with('alert-danger', trans('No results with this search criteria!'));
        }
    }
