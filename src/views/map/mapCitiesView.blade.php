        ...
        <div class="form card">
            ...
            <div class="row">
                <div class="col-12 col-sm-7 col-md-6">
                    ...
                    <a href={{url(strtolower(config('yarm.sys_name')) . '/visualisations/searchFormRefine?form=MapCities')}}>{{Form::button(__('Change Search'),['class'=>'btn btn-primary'])}}</a>
                        ...
                        <div class="card-body p-2">
                            ...
                            <h5 class="card-title">
                                ...
                                <a title="Info" data-toggle="popover" data-trigger="hover"
                                   data-content="@lang('You can set options to change the chart. For Example enable the Toggle 3D option or choose another theme.')"><i
                                        class="fa fa-info-circle"
                                        style="color: grey"></i></a></h5>
                            @foreach($optionsCheckBoxes as $checkbox)
                                <div class="col-3 col-sm-3 form-inline">
                                    <div class="custom-control custom-checkbox ml-2">
                                        {!! Form::checkbox('mapcities_toggle_3d', true , $checkbox['mapcities_toggle_3d_value'], ['id'=>'mapcities_toggle_3d','class' => 'custom-control-input map-select']) !!}
                                        {!! Form::label('mapcities_toggle_3d', __('Toggle 3D'),['class' => 'custom-control-label font-weight-bold']); !!}
                                    </div>
                                </div>
                            @endforeach

                            <div class="current-path hidden" id="MapCities"></div>
                            ...
                            <div style="padding-top: 5%">
                                <a href="{{route('excelMapCities')}}">{{Form::button(__('Export to Excel'),['class'=>'btn btn-primary'])}}</a>
                            </div>
    ...
    @if (!isset($hideListViewDataCounter))
            ...
            @if(Session::has('dataCounterInfo'))
                    @foreach(session('dataCounterInfo') as $sessionArray)
                        ...
                        @else
                            <p class="mb-3"><span class='font-weight-bold'>{{$sessionArray['totalItems']}}</span>@lang('items found for')
                                <span class='font-weight-bold'>{{$sessionArray['searchterm']}}</span>
                            </p>
                        @endif
                    @endforeach
                    @if(count(session('dataCounterInfo')) > 1)
                        <p class="mb-3">@lang('Total items:')
                            ...
                            <span class='font-weight-bold'>{{$ButtonEarliestYear}}</span> @lang('and') <span class='font-weight-bold'>{{$ButtonLatestYear}}</span>
                        </p>
                    @endif
                @endif
                <p style="text-align: center"><a title="Info" data-toggle="popover" data-trigger="hover"
                                                 data-content="@lang('When there is no specific data for your selection, the years will automatically adjust. When activating Toggle 3D you only have to click one time to see the data.')"><i
